...
def test_all():
    ...
    dir = os.path.join(dir, "/gduisgf73r7guai7td7g38yisif7si7/")
    with pytest.raises(DownloadOutPutDirDoesNotExits):
        Download(url="https://example.com", output_dir=dir, youtube_dl_path=None)
    ...
    with pytest.raises(DownloadFailed):
        download2 = Download(
            url=url, output_dir=dir, youtube_dl_path=None
        )  # non downloadable
    ...
    with pytest.raises(YouTubeDLNotFound):
        download2 = Download(
            url=url, output_dir=dir, youtube_dl_path="/home/akamhy/ytdlfake"
    ...
    if which("yt-dlp"):
        ...
        dir = create_and_return_temporary_directory()
        download3 = Download(
            url=url, output_dir=dir, youtube_dl_path=str(which("yt-dlp"))
        )
        file_list = get_list_of_all_files_in_dir(dir)
        total_files = len(file_list)
        if total_files == 0:
    ...
    if which("youtube-dl"):
        ...
        dir = create_and_return_temporary_directory()
        download4 = Download(
            url=url, output_dir=dir, youtube_dl_path=str(which("youtube-dl"))
        )
        file_list = get_list_of_all_files_in_dir(dir)
        total_files = len(file_list)
        if total_files == 0:
